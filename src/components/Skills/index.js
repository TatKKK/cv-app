...
import { fetchSkills, toggleForm, postSkills } from "../..//redux/reducers/skillsSlice";
import Box from "../Box";
    ...
    if (localData) {
        content = 
            localData.map((elem, index) => (
                <div className="skills-elem" key={`${elem.skills?.id}-${index}`} style={ {width: `${elem.skills?.range || 0}%`} }>
                    <span>
                        {elem.skills?.name || 'Unknown'}
                    </span>
                </div>
            ));
    }
